...
const TestServer = function (name) {
    ...
    this.__sendTestRequest = function () {
        ...
        return new Promise((res) => {
            connectionTimeout = setTimeout(() => {
                this.logger(
                    `Test at "${testAddress}" failed - connection timeout.`,
                    'fail',
                    'info'
                );
                res(false);
            ...
            try {
                ...
                testServer
                    .get(testAddress, (response) => {
                        this.logger(
                            `Incoming request (GET) to "${testAddress}".`
                        );
                        let data = '';
                        ...
                        response.on('end', () => {
                            ...
                            this.logger(
                                `Response from "${testAddress}" received: ${data}`,
                                'ok',
                                'info'
                            );
                            res(true);
    ...
    this.__setTestRouting = function () {
        ...
        this.logger(
            `Test router is set on path "${state.path}". Test msg will be served.`
        );
    };
    ...
    this.__testServer = async function () {
        ...
        this.logger(
            `Test success: ${this.expose.serverTestSuccess}.`,
            'ok',
            'info'
        );
    };
