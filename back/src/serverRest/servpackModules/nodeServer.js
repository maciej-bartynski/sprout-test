...
const NodeServer = function (name) {
    ...
    this.__createServerParams = function () {
        ...
        this.logger(
            `Server params created to use. Params length: ${useParams.length}.`
        );
    };
    ...
    this.__createServer = function () {
        ...
        const usePackage = required.protocol() ? required.protocol() : null;
        ...
        setState({
            server: serverPackages[usePackage].createServer(...state.useParams),
            usePackage
        });
    ...
    this.__runServer = function () {
        ...
        return new Promise((res) => {
            portingTimeout = setTimeout(() => {
                this.logger(
                    'Failure: binding port timeout. This is unhandled scenario!',
                    'fail',
                    'info'
                );
                res(false);
            ...
            state.server.listen(required.port(), () => {
                ...
                this.logger(
                    `Server is listening on port: ${required.port()}.`,
                    'ok',
                    'info'
                );
                res(true);
