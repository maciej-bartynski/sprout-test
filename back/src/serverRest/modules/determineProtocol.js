...
const DetermineProtocol = function (name) {
    ...
    this.__whenCertificationFound = function (cert, key) {
        ...
        this.logger(
            'Files considered as certification found. Protocol is set to https.',
            'ok',
            'info'
        );
    };
    ...
    this.__whenCertificationNotFound = function () {
        ...
        this.logger(
            'Files considered as certification not found. Protocol is set to http.'
        );
    };
    ...
    this.__setProtocol = function () {
        ...
        try {
            key = fs.readFileSync(
                path.join(__dirname, '../cert', PRIVATE_KEY_FILE),
                'utf8'
            );
            cert = fs.readFileSync(
                path.join(__dirname, '../cert', CERTIFICATE_FILE),
                'utf8'
            );
        } catch (e) {
            this.logger(
                `Cannot read certification files: ${e}`,
                'warn',
                'info'
            );
        }
